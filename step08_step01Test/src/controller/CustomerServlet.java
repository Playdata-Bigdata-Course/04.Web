...
public class CustomerServlet extends HttpServlet {       
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {		
			...
			request.getSession().setAttribute("allList", CustomerDao.getCustomers());	
			url = "view/list.jsp";

		} catch (SQLException e) {
			e.printStackTrace();
			request.getSession().setAttribute("error", "회원 삭제시 에러");
		}		
		request.getRequestDispatcher(url).forward(request, response);
	}	
	...
	public void update(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{				
			...
			url = "view/updateSuccess.jsp";
		}catch(SQLException e){
			e.printStackTrace();
			request.getSession().setAttribute("error", "업데이트 실패");	
		}
		request.getRequestDispatcher(url).forward(request, response);	
	}
	...
	public void getCustomers(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {			
			...
			request.getSession().setAttribute("error", "모든 고객 검색 실패");
		}
		request.getRequestDispatcher(url).forward(request, response);
	}
}
